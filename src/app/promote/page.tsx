...
// JAVÍTÁS: A progress bar számításához szükséges helper függvény
...
export default function PromotePage() {
  ...
  // ... (a többi logika, useEffect-ek, handlerek változatlanok) ...
  useEffect(() => { miniAppSdk.context.then(ctx => { if (ctx.user?.fid) { setIsAuthenticated(true); setProfile(ctx.user as FarcasterUser); } }); }, []);
  const currentUser = useMemo(() => { if (isAuthenticated && profile) { return { fid: profile.fid, username: profile.username || "user", displayName: profile.displayName || "Current User" }; } return { fid: 0, username: "guest", displayName: "Guest" }; }, [isAuthenticated, profile]);
  const fetchShareTimers = useCallback(async () => { /* ... */ }, [currentUser]);
  const fetchPromotions = useCallback(async () => { /* ... */ }, []);
  const fetchUserStats = useCallback(async () => { /* ... */ }, [currentUser]);
  const refreshAllData = useCallback(async () => { setLoading(true); await Promise.all([ fetchPromotions(), fetchUserStats(), fetchShareTimers() ]); setLoading(false); }, [fetchPromotions, fetchUserStats, fetchShareTimers]);
  useEffect(() => { if (isAuthenticated && profile) { refreshAllData(); const interval = setInterval(fetchShareTimers, 60000); return () => clearInterval(interval); } }, [isAuthenticated, profile, refreshAllData, fetchShareTimers]);
  ...
  const sortedAvailablePromos = useMemo(() => { /* ... */ }, [availablePromos, shareTimers]);
                    ...
                    sortedAvailablePromos.map((promo) => {
                      ...
                      return (
                        <div key={promo.id} className="bg-[#181c23] p-4 rounded-lg border border-gray-700 flex flex-col gap-4">
                          ...
                          {/* JAVÍTÁS: Progress bar a kampány állapotának vizualizálásához */}
