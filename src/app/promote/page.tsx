...
export default function PromotePage() {
  ...
  useEffect(() => {
    miniAppSdk.context.then(ctx => {
      if (ctx.user?.fid) {
        ...
        // Ha a context betöltődött, de nincs user, akkor is fejezzük be a töltést
        setLoading(false);
      }
    });
  }, []);
  ...
  // JAVÍTÁS: Egyetlen, robusztus adatlekérő useEffect
  useEffect(() => {
    const fetchAllData = async () => {
      setLoading(true);
      try {
        // A promóciókat mindig lekérjük
        const promosPromise = fetch("/api/promotions?status=all").then(res => res.json());

        // A felhasználói adatokat csak akkor, ha van profil
        const statsPromise = profile?.fid ? fetch(`/api/users/${profile.fid}`).then(res => res.json()) : Promise.resolve(null);
        const timersPromise = profile?.fid ? fetch(`/api/share-timers?fid=${profile.fid}`).then(res => res.json()) : Promise.resolve(null);
        
        const [promoData, statsData, timersData] = await Promise.all([promosPromise, statsPromise, timersPromise]);

        if (promoData?.promotions) {
            setPromoCasts(promoData.promotions.map(convertDbToPromoCast));
        }

        if (statsData?.user) {
            setUserStats({
                totalEarnings: statsData.user.total_earnings,
                totalShares: statsData.user.total_shares,
                pendingClaims: statsData.user.pending_claims
            });
        }

        if (timersData?.timers) {
            const timersMap = timersData.timers.reduce((acc: any, timer: any) => {
    ...
    // Az időzítők időszakos frissítése
    const interval = setInterval(() => {
        if(profile?.fid) {
            fetch(`/api/share-timers?fid=${profile.fid}`).then(res => res.json()).then(data => {
                if (data?.timers) {
  ...
  }, [profile]); // Ez a hook CSAK akkor fut le, ha a profil (bejelentkezés) megváltozik.
  
  const handleCreateSuccess = () => { setShowForm(false); if (profile) { /* Újra kéne hívni a fetchAllData-t */ } };
  const handleCreateCancel = () => { setShowForm(false); };
  const handleFundSuccess = () => { setShowFundingForm(false); setFundingPromo(null); if (profile) { /* Újra kéne hívni a fetchAllData-t */ } };
  const handleFundCancel = () => { setShowFundingForm(false); setFundingPromo(null); };

  const handleSharePromo = async (promo: PromoCast) => {
    // ... (a megosztás logikája változatlan, de a végén a refreshAllData helyett csak a state-eket frissíthetnénk optimistábban)
  };
