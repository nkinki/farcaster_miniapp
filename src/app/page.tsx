    ...
    allViewRows = sortedMiniapps.map((app: Miniapp, idx: number) => {
      ...
      return (
        <>
          ...
          <div key={app.rank} className={`flex items-center justify-between rounded-xl px-3 py-2 ${highlight} border border-[#23283a] shadow-sm ${favorites.includes(app.domain) ? 'ring-2 ring-blue-400/80 shadow-[0_0_12px_2px_rgba(0,200,255,0.5)]' : ''}`}> 
            <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-base mr-2 bg-gray-700 text-white`}>{app.rank}</div>
            {app.iconUrl ? (
              <img
                src={app.iconUrl}
      ...
      <nav className="fixed bottom-0 left-0 w-full z-50 bg-black/95 shadow-2xl border-t-2 border-gray-800">
        <div className="flex w-full max-w-3xl mx-auto px-0 pb-0 pt-0">
          ...
          <a
            ...
            className="flex-1 py-2 font-bold text-[0.65rem] focus:outline-none focus:ring-2 focus:ring-pink-400 bg-gray-300 text-black hover:bg-gray-200 transition-all duration-200 flex flex-col items-center justify-center border-t border-r border-gray-400/60"
