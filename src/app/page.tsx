  ...
  if (filter !== 'all') {
    ...
    categoryViewRows = [
      ...((favoriteMiniapps.map(app => {
        ...
        return (
          <div key={app.domain + '-favtop'} className={`flex items-center justify-between rounded-xl px-3 py-2 bg-[#181c23] border-2 border-blue-400 shadow-sm ring-2 ring-blue-400/80 shadow-[0_0_12px_2px_rgba(0,200,255,0.5)]`}>
            <div className="flex-shrink-0 w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg bg-gray-700 text-white mr-2">{idx + 1}</div>
            {app.iconUrl ? (
              <img
                src={app.iconUrl}
                alt={app.name + ' logo'}
      ...
      ...categoryMiniapps.map((app, idx) => {
        ...
        return (
          <div key={app.domain} className={`flex items-center justify-between rounded-xl px-3 py-2 bg-[#181c23] border border-[#2e3650] shadow-sm`}>
            <div className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-lg bg-gray-700 text-white mr-2">{idx + 1}</div>
            {app.iconUrl ? (
              <img
                src={app.iconUrl}
                alt={app.name + ' logo'}
      ...
      return (
        <>
          ...
          <div key={app.rank} className={`flex items-center justify-between rounded-xl px-3 py-2 ${highlight} border border-[#23283a] shadow-sm ${favorites.includes(app.domain) ? 'border-2 border-blue-400 ring-2 ring-blue-400/80 shadow-[0_0_12px_2px_rgba(0,200,255,0.5)]' : ''}`}> 
            <div className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-lg bg-gray-700 text-white mr-2" style={{fontSize: '2em'}}>{app.rank}</div>
            {app.iconUrl ? (
              <img
                src={app.iconUrl}
                alt={app.name + ' logo'}
