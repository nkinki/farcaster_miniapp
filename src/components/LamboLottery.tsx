...
import { FiX, FiDollarSign, FiUsers, FiZap } from "react-icons/fi"
...
export default function LamboLottery({ isOpen, onClose, userFid, onPurchaseSuccess }: LamboLotteryProps) {
    ...
    const verifyAndRegister = async () => {
      ...
      try {
        const response = await fetch(
          `/api/lottery/verify-purchase?txHash=${purchaseTxHash}&fid=${userFid}&round_id=${currentRound!.id}&playerAddress=${address}&ticket_numbers=${selectedNumbers.join(",")}`,
          {
            method: "GET",
          },
        )
        ...
        console.log("[v0] Purchase verified successfully:", result)
        ...
        setErrorMessage(
          `CRITICAL ERROR: Purchase successful, but registration failed. Contact support with TxHash: ${purchaseTxHash}.`,
        )
        setStep(PurchaseStep.ReadyToPurchase)
      }
    }
  ...
  const handleNumberSelect = (number: number) => {
    ...
    } else if (userTickets.length + selectedNumbers.length < 10) {
            ...
            <div className="w-full flex justify-between items-start mb-2">
              <div className="flex items-center gap-4">
                <div className="w-full">
                  ...
                  {currentRound && (
                    <div
                      className="mt-4 w-full max-w-full py-3 px-6 bg-gradient-to-r from-yellow-900/30 to-orange-900/30 border-2 border-yellow-400/50 rounded-xl animate-pulse shadow-[0_0_25px_rgba(255,255,0,0.4)] pulse-glow mx-auto"
                      style={{ animationDuration: "4s" }}
                    >
                      <div className="w-full grid grid-cols-3 items-center justify-items-center gap-4">
                        <div className="text-center min-w-0">
                          <div className="text-xs font-bold text-yellow-300 mb-1">TIME LEFT</div>
                        ...
                        <div className="text-center border-l-2 border-r-2 border-yellow-400/30 px-4 min-w-0 w-full">
                          <div className="text-xs font-bold text-yellow-300 mb-1">JACKPOT</div>
                          <div
                            className="text-lg font-bold text-cyan-300 animate-pulse drop-shadow-[0_0_10px_rgba(34,211,238,0.9)]"
                            style={{ animationDuration: "4s" }}
                          >
                        ...
                        <div className="text-center min-w-0">
                          <div className="text-xs font-bold text-yellow-300 mb-1">LAST DRAW</div>
                          <div className="text-base font-bold text-cyan-300 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]">
                            {lastWinningNumber || "N/A"}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-3 bg-yellow-900/20 border border-yellow-500/30 rounded-lg">
                    ...
                    {selectedNumbers.length > 0 && (
                      <div className="text-sm text-yellow-300 mt-1">
                        <span className="font-bold">Total cost:</span> {(Number(totalCost) / 1e18).toLocaleString()}{" "}
                        CHESS
                      </div>
                    )}
                  </div>
                  {isConnected && (
                    <div className="p-3 bg-green-900/20 border border-green-500/30 rounded-lg">
                      <div className="text-sm font-medium text-green-300">Token Approval Status</div>
                      <div className="text-xs text-gray-400">
                        {step === PurchaseStep.ReadyToPurchase
                          ? "Sufficient allowance approved."
                          : "Approval will be needed to purchase."}
                      </div>
                    </div>
                  )}
                </div>
                  ...
                  <p className="text-sm text-blue-300">
                    ...
                    {userTickets.length > 0 && (
                      <span className="block mt-1">
                        You already have <span className="font-bold text-yellow-300">{userTickets.length}/10</span>{" "}
                        tickets.
                      </span>
                    )}
                  </p>
                ...
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-gray-300">
                    <div>
                      ...
                      <span className="text-yellow-400 font-bold">
                        {(Number(totalCost) / 1e18).toLocaleString()} CHESS
                      </span>
                    </div>
                  ...
                  {step < PurchaseStep.ReadyToPurchase ? (
                    <button
                      ...
                      className="px-6 py-3 rounded-xl font-bold text-lg transition-all duration-300 disabled:bg-gray-600 disabled:cursor-not-allowed bg-gradient-to-r from-blue-600 to-purple-600 text-white"
                    >
                    ...
                    <button
                      ...
                      disabled={isLoading || !isConnected || selectedNumbers.length === 0}
                      className="px-6 py-3 rounded-xl font-bold text-lg transition-all duration-300 disabled:bg-gray-600 disabled:cursor-not-allowed bg-gradient-to-r from-green-600 to-blue-600 text-white"
                    >
                      {isPurchased && step !== PurchaseStep.Saving
                        ? "Success!"
                        : isPurchaseConfirming
                          ? "Confirming..."
                          : isPending
                            ? "Check Wallet..."
                            : `2. Buy ${selectedNumbers.length} Ticket(s)`}
                    </button>
                  )}
                </div>
              </div>
