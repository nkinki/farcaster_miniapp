...
async function main() {
    ...
    try {
        const destinationWallet = await publicClient.readContract({
            address: LOTTO_PAYMENT_ROUTER_ADDRESS,
            abi: LOTTO_PAYMENT_ROUTER_ABI,
            functionName: 'destinationWallet'
        });
        console.log(`The current destinationWallet for LottoPaymentRouter is: ${destinationWallet}`);
    } catch (error) {
