...
async function main() {
    ...
    try {
        ...
        // Get Jackpot
        const result = await pool.query(`SELECT jackpot FROM lottery_draws WHERE status = 'active' ORDER BY id DESC LIMIT 1`);
        // Get Active Daily Code
        try {
            const codeResult = await pool.query(`SELECT code FROM lotto_daily_codes WHERE is_active = TRUE ORDER BY created_at DESC LIMIT 1`);
 ...
 */
async function sendNotification(tokenRows, title, body, notificationType) {
    const urlMap = {};
